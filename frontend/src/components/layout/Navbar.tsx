      ...
      <header className="fixed top-0 inset-x-0 z-40 nav-surface">
        <nav className="max-w-7xl mx-auto px-6 sm:px-8 h-16 flex items-center gap-4">
          ...
          <Link to="/" className="flex items-center gap-2.5 shrink-0">
            <div className="w-9 h-9 rounded-xl bg-primary flex items-center justify-center text-primary-foreground font-black text-base shadow-sm">K</div>
            <span className="font-black text-xl tracking-tight hidden sm:block" style={{ color: 'hsl(var(--nav-fg))' }}>Kumbi</span>
          </Link>
          ...
          <div className="hidden md:flex items-center gap-0.5 flex-1 justify-center">
            {navLinks.map((l) => (
              <Link
                key={l.to}
                to={l.to}
                className={cn('nav-link px-4 py-2 rounded-lg text-sm font-semibold transition-all', isActive(l.to) && 'nav-link-active')}
              >
                {l.label}
              </Link>
            ))}
      ...
      <AnimatePresence>
        {sidebarOpen && (
          <>
            ...
            >
              <div className="flex items-center justify-between px-6 h-16 border-b" style={{ borderColor: 'hsl(var(--nav-border))' }}>
                <Link to="/" className="flex items-center gap-2.5">
                  <div className="w-9 h-9 rounded-xl bg-primary flex items-center justify-center text-primary-foreground font-black text-base">K</div>
                  <span className="font-black text-xl tracking-tight" style={{ color: 'hsl(var(--nav-fg))' }}>Kumbi</span>
                </Link>
                <button
                  onClick={() => setSidebarOpen(false)}
                  className="w-9 h-9 flex items-center justify-center rounded-xl hover:bg-primary/10 transition-colors"
              ...
              <nav className="flex-1 overflow-y-auto px-4 py-6 flex flex-col gap-1">
                {navLinks.map((l) => (
                  <Link
                    key={l.to}
                    to={l.to}
                    className={cn('nav-link flex items-center px-4 py-3.5 rounded-xl text-base font-semibold transition-all', isActive(l.to) && 'nav-link-active')}
                  >
                    {l.label}
                  </Link>
                ))}
                <div className="my-2 border-t" style={{ borderColor: 'hsl(var(--nav-border))' }} />
                <button
                  onClick={() => { openContact(); setSidebarOpen(false) }}
                  className="nav-link flex items-center px-4 py-3.5 rounded-xl text-base font-semibold text-left w-full"
                >
                  Contact Us
                </button>
                {isAuthenticated ? (
                  <Link to="/cms" className="nav-link nav-link-active flex items-center px-4 py-3.5 rounded-xl text-base font-semibold">CMS Dashboard ↗</Link>
                ) : (
                  <button
              ...
              <div className="px-4 pb-6 pt-4 flex flex-col gap-3 border-t" style={{ borderColor: 'hsl(var(--nav-border))' }}>
                <button onClick={() => { openVolunteer(); setSidebarOpen(false) }} className="btn-primary w-full">
