...
import { ExternalLink, LayoutGrid, Plus, Save, Trash2 } from "lucide-react";
import { useState } from "react";
import { configApi } from "@/api/client";
import ImagePicker from "@/components/ui/ImagePicker";
import RichTextarea from "@/components/ui/RichTextarea";
import { Skeleton } from "@/components/ui/Skeleton";
import type { ContentCard, ProjectItem, SiteConfig } from "@/hooks/useConfig";
...
function ProjectEditor({
  ...
  const set = (k: keyof ProjectItem) => (v: string) =>
    onChange({ ...item, [k]: v });
  return (
    <div className="glass-card p-5 flex flex-col gap-4">
      <p className="font-bold text-sm text-primary uppercase tracking-widest">
        {item.id}
      </p>
      <Field label="Title">
        <TextInput value={item.title} onChange={set("title")} />
      </Field>
      <Field label="Tag / Category">
        <TextInput
          value={item.tag}
          onChange={set("tag")}
          placeholder="e.g. Missing Persons · Data"
        />
      </Field>
      <Field label="Description">
        <RichTextarea
          key={`proj-desc-${item.id}`}
    ...
    <div className="flex flex-col gap-4">
      {cards.map((c, i) => (
        <CardEditor
          ...
          onDelete={() => onChange(cards.filter((_, idx) => idx !== i))}
        />
      ))}
      <button
        onClick={addCard}
        className="btn-ghost gap-2 self-start text-sm font-semibold"
      >
...
function NavSection({
  ...
  cfg: SiteConfig;
  set: ReturnType<typeof useSectionSet>;
}) {
  return (
    <div className="glass-card p-6 flex flex-col gap-5">
      <Field label="Brand Name">
        <TextInput
          value={cfg.nav.brand}
          onChange={(v) => set("nav")({ brand: v })}
        />
      </Field>
      <Field label="Tagline">
        <TextInput
          value={cfg.nav.tagline}
          onChange={(v) => set("nav")({ tagline: v })}
        />
      </Field>
    </div>
  );
    ...
    <div className="glass-card p-6 flex flex-col gap-5">
      <Field label="Heading">
        <TextInput
          ...
          onChange={(v) => set("hero")({ heading: v })}
        />
      </Field>
      <Field label="Badge (pill above heading)">
        <TextInput
          value={cfg.hero.badge}
          onChange={(v) => set("hero")({ badge: v })}
          placeholder="e.g. Kenya · Community · Impact"
        />
      </Field>
      <Field label="Subheading">
        <RichTextarea
          key="hero-sub"
          initialContent={cfg.hero.subheading}
...
function ProjectsSection({
  ...
  onAddProject: () => void;
  onDeleteProject: (idx: number) => void;
}) {
  return (
    <div className="glass-card p-6 flex flex-col gap-5">
      <Field label="Section Heading">
        <TextInput
          value={cfg.projects.heading}
      ...
      <Field label="Section Subheading">
        <RichTextarea
          key="proj-sub"
          initialContent={cfg.projects.subheading}
          onChange={(v) => set("projects")({ subheading: v })}
          placeholder="Projects subheading…"
        />
      </Field>
      <Field label="Tagline (above heading)">
        <TextInput
          value={cfg.projects.tagline}
          onChange={(v) => set("projects")({ tagline: v })}
        />
      </Field>
      <ImagePicker
        label="Background Image"
        value={cfg.projects.backgroundImage}
        onChange={(v) => set("projects")({ backgroundImage: v })}
      />
      ...
      {cfg.projects.items.map((item, i) => (
        <div key={item.id} className="relative">
          <button
            ...
            className="absolute top-5 right-5 z-10 p-1.5 rounded-lg text-muted-foreground hover:text-destructive hover:bg-destructive/10 transition-colors"
          ...
          <ProjectEditor
            item={item}
            onChange={(val) => setProjectItem(i, val)}
          />
        </div>
      ))}
      <button
...
function VolunteerSection({
  ...
  return (
    <div className="glass-card p-6 flex flex-col gap-5">
      <Field label="Heading">
        <TextInput
          value={cfg.volunteer.heading}
          onChange={(v) => set("volunteer")({ heading: v })}
        />
      </Field>
      <Field label="Subheading">
        <RichTextarea
          key="vol-sub"
          initialContent={cfg.volunteer.subheading}
          onChange={(v) => set("volunteer")({ subheading: v })}
          placeholder="Volunteer subheading…"
        />
      </Field>
      <Field label="Button Text">
        <TextInput
          value={cfg.volunteer.cta}
          onChange={(v) => set("volunteer")({ cta: v })}
        />
      </Field>
      <ImagePicker
        label="Background Image"
        value={cfg.volunteer.backgroundImage}
        onChange={(v) => set("volunteer")({ backgroundImage: v })}
      />
    </div>
  );
}
  ...
  return (
    <div className="glass-card p-6 flex flex-col gap-5">
      <Field label="About Text">
        <RichTextarea
          key="footer-about"
          initialContent={cfg.footer.about}
          onChange={(v) => set("footer")({ about: v })}
      ...
      <div className="grid sm:grid-cols-2 gap-4">
        ...
        <Field label="City / Country">
          <TextInput
            value={cfg.footer.city}
            onChange={(v) => set("footer")({ city: v })}
          />
        </Field>
        <Field label="Email">
          <TextInput
  ...
  return (
    <div className="glass-card p-6 flex flex-col gap-5">
      <p className="text-sm text-muted-foreground">
        Set your organisation's social media profile URLs. Leave blank to hide
        the icon in the footer.
      </p>
      <Field label="X (Twitter) URL">
        <div className="flex gap-2 items-center">
          <TextInput
            value={cfg.footer.twitter}
            onChange={(v) => set("footer")({ twitter: v })}
            placeholder="https://x.com/kumbi"
          />
          {cfg.footer.twitter && (
            <a
              href={cfg.footer.twitter}
              target="_blank"
      ...
      <Field label="Instagram URL">
        <div className="flex gap-2 items-center">
          <TextInput
            ...
            onChange={(v) => set("footer")({ instagram: v })}
            placeholder="https://instagram.com/kumbi"
          />
          {cfg.footer.instagram && (
            <a
              href={cfg.footer.instagram}
              target="_blank"
              rel="noopener noreferrer"
              className="p-2 hover:text-primary transition-colors"
            >
      ...
      <Field label="Facebook URL">
        <div className="flex gap-2 items-center">
          <TextInput
            ...
            onChange={(v) => set("footer")({ facebook: v })}
            placeholder="https://facebook.com/kumbi"
          />
          {cfg.footer.facebook && (
            <a
              href={cfg.footer.facebook}
              target="_blank"
              rel="noopener noreferrer"
              className="p-2 hover:text-primary transition-colors"
            >
...
function AboutValuesSection({
  ...
  cfg: SiteConfig;
  setPage: (
    page: keyof SiteConfig["pages"],
  ) => (patch: Partial<SiteConfig["pages"][typeof page]>) => void;
}) {
  return (
    <div className="glass-card p-6 flex flex-col gap-5">
      <p className="form-label">About Us — Values Cards</p>
      <CardsSection
        cards={cfg.pages.about.values}
        onChange={(v) => setPage("about")({ values: v })}
      />
    </div>
  );
}

function TraceFeaturesSection({
  cfg,
  setPage,
}: {
  cfg: SiteConfig;
  ...
  return (
    <div className="glass-card p-6 flex flex-col gap-5">
      ...
      <CardsSection
        cards={cfg.pages.trace.features}
        onChange={(v) => setPage("trace")({ features: v })}
      />
    </div>
  );
}

function VoteFeaturesSection({
  cfg,
  setPage,
}: {
  cfg: SiteConfig;
  ...
  return (
    <div className="glass-card p-6 flex flex-col gap-5">
      ...
      <CardsSection
        cards={cfg.pages.vote.features}
        onChange={(v) => setPage("vote")({ features: v })}
      />
    </div>
  );
}
...
function VolunteerRolesSection({
  ...
  return (
    <div className="glass-card p-6 flex flex-col gap-5">
      ...
      <CardsSection
        cards={cfg.pages.volunteer.roles}
        onChange={(v) => setPage("volunteer")({ roles: v })}
      />
    </div>
  );
}
  ...
  return (
    <div className="glass-card p-6 flex flex-col gap-5">
      ...
      <Field label="Hero Tag / Badge">
        <TextInput
          value={p.heroTag}
          onChange={(v) => setPage(page)({ heroTag: v })}
      ...
      <Field label="Hero Subheader">
        <RichTextarea
          key={`${page}-sub`}
          initialContent={p.subheading}
          onChange={(v) => setPage(page)({ subheading: v })}
          placeholder="Hero subheader text…"
        />
      </Field>
      <ImagePicker
        label="Hero Image"
        value={p.heroImage}
        onChange={(v) => setPage(page)({ heroImage: v })}
      />
...
function useSectionSet(
  setDraft: React.Dispatch<React.SetStateAction<SiteConfig | null>>,
) {
...
export default function SiteContent() {
  ...
  const firstSection = (
    SECTIONS.find((s): s is { id: string; title: string } => "id" in s) ?? {
      id: "nav",
    }
  ).id;
  ...
  if (raw && !draft)
    setDraft({
      ...
      projects: {
        ...DEFAULTS.projects,
        ...(raw.projects || {}),
        items: raw.projects?.items || DEFAULTS.projects.items,
      },
      volunteer: { ...DEFAULTS.volunteer, ...(raw.volunteer || {}) },
      footer: { ...DEFAULTS.footer, ...(raw.footer || {}) },
      correspondenceEmail:
        raw.correspondenceEmail || DEFAULTS.correspondenceEmail || "",
      pages: {
        about: {
          ...DEFAULTS.pages.about,
          ...(raw.pages?.about || {}),
          values: raw.pages?.about?.values || DEFAULTS.pages.about.values,
        },
        projects: {
          ...DEFAULTS.pages.projects,
          ...(raw.pages?.projects || {}),
        },
        blog: { ...DEFAULTS.pages.blog, ...(raw.pages?.blog || {}) },
        volunteer: {
          ...DEFAULTS.pages.volunteer,
          ...(raw.pages?.volunteer || {}),
          roles: raw.pages?.volunteer?.roles || DEFAULTS.pages.volunteer.roles,
        },
        trace: {
          ...DEFAULTS.pages.trace,
          ...(raw.pages?.trace || {}),
          features: raw.pages?.trace?.features || DEFAULTS.pages.trace.features,
        },
        vote: {
          ...DEFAULTS.pages.vote,
          ...(raw.pages?.vote || {}),
          features: raw.pages?.vote?.features || DEFAULTS.pages.vote.features,
        },
      },
    });
    ...
    (patch: Partial<SiteConfig["pages"][typeof page]>) =>
      setDraft((d) =>
        d
          ? {
              ...d,
              pages: { ...d.pages, [page]: { ...d.pages[page], ...patch } },
            }
          : d,
      );
  ...
  const onAddProject = () =>
    setDraft((d) => {
      ...
      return {
        ...d,
        projects: { ...d.projects, items: [...d.projects.items, newItem] },
      };
    });
      ...
      <div className="flex gap-6">
        <div className="flex-1 min-w-0">
          ...
          {activeSection === "volunteer" && (
            <VolunteerSection cfg={cfg} set={set} />
          )}
          {activeSection === "footer" && (
            <FooterSection
              ...
              set={set}
              setTop={(v) =>
                setDraft((d) => (d ? { ...d, correspondenceEmail: v } : d))
              }
            />
          )}
          {activeSection === "footer-social" && (
            <SocialLinksSection cfg={cfg} set={set} />
          )}
          {activeSection === "about" && (
            <PageSection cfg={cfg} page="about" setPage={setPage} />
          )}
          {activeSection === "about-values" && (
            <AboutValuesSection cfg={cfg} setPage={setPage} />
          )}
          {activeSection === "projects-page" && (
            <PageSection cfg={cfg} page="projects" setPage={setPage} />
          )}
          {activeSection === "blog-page" && (
            <PageSection cfg={cfg} page="blog" setPage={setPage} />
          )}
          {activeSection === "trace" && (
            <PageSection cfg={cfg} page="trace" setPage={setPage} />
          )}
          {activeSection === "trace-features" && (
            <TraceFeaturesSection cfg={cfg} setPage={setPage} />
          )}
          {activeSection === "vote" && (
            <PageSection cfg={cfg} page="vote" setPage={setPage} />
          )}
          {activeSection === "vote-features" && (
            <VoteFeaturesSection cfg={cfg} setPage={setPage} />
          )}
          {activeSection === "volunteer-page" && (
            <PageSection cfg={cfg} page="volunteer" setPage={setPage} />
          )}
          {activeSection === "volunteer-roles" && (
            <VolunteerRolesSection cfg={cfg} setPage={setPage} />
          )}
        </div>
      </div>
        ...
        {sectionOpen && (
          <>
            ...
            <div className="absolute top-16 right-0 z-40 w-64 glass-card overflow-y-auto p-3 border border-border shadow-2xl rounded-2xl max-h-[70vh]">
              <p className="text-xs font-bold uppercase tracking-widest text-muted-foreground px-3 py-2">
                Sections
              </p>
              {SECTIONS.map((s) => {
                if ("separator" in s) {
                  return (
                    <p
                      key={s.separator}
                      className="text-xs font-bold uppercase tracking-widest text-muted-foreground px-3 pt-4 pb-1.5"
                    >
                      {s.separator}
                    </p>
                  );
